...
\begin{resultbox}{Weak Boson Mass Ratio via Stiffness}
\begin{equation}
\frac{m_W}{m_Z} = \sqrt{\frac{2G}{2G(1+\nu_{vac})}} = \frac{1}{\sqrt{1+\nu_{vac}}}
\end{equation}
\end{resultbox}

By substituting the geometric Chiral LC trace-reversed limit derived in Chapter 4 ($\nu_{vac} \equiv 2/7$), the weak mixing angle emerges as an analytical prediction:

\begin{resultbox}{The Weak Mixing Angle}
\begin{equation}
...
\begin{resultbox}{On-Shell Weak Mixing Angle}
\begin{equation}
    \sin^2\theta_W^{\text{on-shell}} \equiv 1 - \left(\frac{M_W}{M_Z}\right)^2 = 1 - \frac{7}{9} = \frac{2}{9} \approx 0.2222
\end{equation}
\end{resultbox}
This matches the PDG on-shell value ($0.2230$)~\cite{pdg2022} to within $0.35\%$. The commonly quoted MSbar value ($0.2312$) incorporates radiative corrections and is a distinct quantity.
...
The electroweak sector is derived in full in Chapter \ref{ch:electroweak_higgs}. Here the text summarizes only the critical results that interlock with the gauge structure derived in this chapter.
...
\begin{resultbox}{Analytical On-Shell Weak Mixing Angle}
\begin{equation}
    \sin^2\theta_W = 1 - \frac{7}{9} = \frac{2}{9} \approx 0.2222 \qquad (\text{PDG on-shell: } 0.2230~\cite{pdg2022}, \; -0.35\%)
\end{equation}
\end{resultbox}

The W and Z masses follow from the torsional self-energy of the unknot:
\begin{resultbox}{The W and Z Boson Masses}
\begin{equation}
...
\begin{resultbox}{Schwinger Anomalous Magnetic Moment}
\begin{equation}
...
\begin{resultbox}{Angular Momentum Coherence Length}
\begin{equation}
...
\begin{resultbox}{The Self-Trapping Frequency}
\begin{equation}
    f_C = \frac{m_e c^2}{\hbar} \approx 1.24 \times 10^{20} \text{ Hz},
\end{equation}
\end{resultbox}
the helix radius collapses to
\begin{resultbox}{Helix Self-Closing Radius}
\begin{equation}
...
\end{itemize}
The fine structure constant $\alpha = (V_{\text{peak}}/V_{\text{snap}})^2 / (4\pi)$ measures \emph{how much} the trapped photon strains the lattice node it sits on. The entire spectrum of matter---from free photons to confined electrons---is a single excitation at different frequencies, separated by the self-trapping threshold at $f = f_C$.
