...
\midrule
$\delta_{th}$ & $\dfrac{1}{28\pi}
    = \dfrac{\nu_{vac}}{\kappa_{cold}}$
    & 0.01137
    & Thermal softening fraction \\[6pt]
...
\subsection{Thermal Softening ($\delta_{th} = 1/(28\pi)$)}
...
gradient tensor.  The softening fraction is the ratio of the
lattice's compliance ($\nu_{vac} = 2/7$, the Poisson ratio) to its
Skyrme stiffness ($\kappa_{cold} = 8\pi$, the Faddeev--Skyrme coupling):
\begin{equation}
    \delta_{th} = \frac{\nu_{vac}}{\kappa_{cold}}
    = \frac{2/7}{8\pi} = \frac{1}{28\pi} \approx 0.01137
\end{equation}
...
The Golden Ratio is the icosahedron. The Fibonacci sequence is an icosahedral approximation algorithm. $\pi$ is Gauss's law. The magic numbers are geometric shell closures. $g_* = 7^3/4$ is the lattice mode count. $\alpha_s = \alpha^{3/7}$ is the spatial compliance projection. $\lambda_H = 1/8$ is the K4 breathing mode.
