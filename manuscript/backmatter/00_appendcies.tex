\appendix
\chapter{Unified Equation Set}
\label{app:unified_equations}

\section{The Rosetta Stone of Vacuum Engineering}
\label{sec:rosetta_stone_constants}

The following table summarizes the fundamental constants of nature as derived from the hardware specifications of the Discrete Amorphous Manifold ($M_A$).
...
\begin{tabular}{|l|c|l|}
\hline
\textbf{Constant} & \textbf{AVE Derivation} & \textbf{Physical Meaning} \\ \hline
Fine Structure ($\alpha^{-1}$) & $4\pi^3 + \pi^2 + \pi$ & Geometric Impedance of Spin-1/2 Trefoil \\ \hline
Mass Scale ($m$) & $m \propto N^9$ & Hyper-Elastic Quartic Vacuum Strain \\ \hline
Gravitational Constant ($G$) & $\frac{c^4}{\kappa \epsilon_0 V_0^2}$ & Mechanical Compliance of the Lattice \\ \hline
Dark Matter Accel ($a_0$) & $\frac{c H_0}{2\pi}$ & Kinematic Drift from Node Genesis \\ \hline
Vacuum Viscosity ($\eta$) & $\alpha \frac{\hbar}{l_0^3}$ & Quantum Granularity Drag \\ \hline
\end{tabular}
\caption{The Rosetta Stone: Mapping abstract constants to hardware specifications.}
\label{tab:rosetta_constants}
\end{table}
...
\lstinputlisting[language=Python, caption=Mass Hierarchy Derivation (simulations/run\_derive\_mass\_scaling.py), basicstyle=\ttfamily\footnotesize, breaklines=true]{chapters/13_thermodynamic_cycle/simulations/run_derive_mass_scaling.py}
...
\lstinputlisting[language=Python, caption=Lid-Driven Cavity Solver (simulations/run\_lid\_driven\_cavity.py), basicstyle=\ttfamily\footnotesize, breaklines=true]{chapters/10_vacuum_cfd/simulations/run_lid_driven_cavity.py}
...
\begin{table}[h]
    ...
    \begin{tabularx}{\textwidth}{@{}l|X@{}}
        ...
        \bottomrule
    \end{tabularx}
    \caption{The Dictionary of Applied Vacuum Engineering}
\end{table}
