...
\begin{table}[htbp]
    ...
    \multicolumn{3}{c}{\cellcolor[HTML]{EFEFEF} \textbf{Orbital Mechanics \& Cosmology}} \\
    ...
    \multicolumn{3}{c}{\cellcolor[HTML]{EFEFEF} \textbf{Quantum Mechanics \& Atomic Physics}} \\
    Schrödinger Wave Equation & Helmholtz Continuous LC Fluid Dynamics & Acoustic Pressure Cavity Modes \\
    Atomic Orbital / Electron Cloud & Macroscopic Impedance Cavity & Resonant Standing Wave Topology \\
    Ionization Energy (IE) & Outermost Mode Eigenvalue & Regime Boundary $S \to 0$ Gap \\
    Principal Quantum Number ($n$) & Phase-Locking Integer ($\int n(r) dr$) & Optical Path Length Resonance \\
    Electron-Electron Screening & $Z_{eff}(r)$ Strain Superposition & Local Impedance Bumps \\

    \multicolumn{3}{c}{\cellcolor[HTML]{EFEFEF} \textbf{Topological Chemistry \& Molecular Biology}} \\
    Ramachandran Torsional Rigidity ($\phi, \psi$) & $p$-Shell Macro Equivalent Phase Bound ($120^\circ$) & Rigid Polypeptide Steric Hindrance \\
    Ramachandran Disallowed Basins ($\phi > 0$)  & Reactive Impedance Tuning Stubs ($\Gamma \to -1$) & RF Smith Chart Forbidden Phase Zones \\
    Hydrogen Bond Acceptor/Donor Coupling & Dual-Microstrip Even/Odd Admittance Pair & Macromolecular Secondary Structure ($\alpha$, $\beta$) \\
