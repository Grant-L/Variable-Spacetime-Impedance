...
\paragraph{The Poisson-Disk Solution:} To satisfy macroscopic isotropy while strictly enforcing the microscopic hardware cutoff, the software must generate the node coordinates using a \textbf{Poisson-Disk Hard-Sphere Sampling Algorithm}. By strictly enforcing an exclusion radius of $r_{min} = l_{node}$ during genesis, the lattice naturally settles into a packing fraction of $\approx 0.17-0.18$, creating a stable, sparse dielectric substrate.

\paragraph{Rheological Tuning:} Simulation confirms that the "Trace-Reversed" mechanical state ($K=2G$) is an emergent property of the Cosserat coupling modulus.
\begin{itemize}
