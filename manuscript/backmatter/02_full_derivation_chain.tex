...
\textbf{Scorecard:}
\begin{itemize}
    \item \textbf{Derived (\checkmark):} 17 of 26 --- $\alpha$, $\sin^2\theta_W$,
          $\alpha_s$, $m_e$, $m_\mu$, $m_\tau$, $m_{\nu_1}$, $m_{\nu_2}$,
          $m_{\nu_3}$, $v$, $m_H$, 4 CKM parameters ($\lambda$, $A$, $\rho$,
          $\eta$), and the derived observables $G_F$, $\theta_{QCD}$.
    \item \textbf{Structurally eliminated ($\varnothing$):} 1 --- $\theta_{QCD}$.
    \item \textbf{Future targets ($\triangleright$):} 9 --- 6 quark masses
          and 3 PMNS mixing angles + phase.
\end{itemize}
\noindent
...
\textbf{Predictions within 5\% of measurement:} 30/30.\\
\textbf{SM parameters derived or eliminated:} 17/26 (Table~\ref{tab:sm_params}).
