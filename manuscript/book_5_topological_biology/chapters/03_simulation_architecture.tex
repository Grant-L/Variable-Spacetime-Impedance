...
\paragraph{Segment impedances from shared electrons.}
The characteristic impedance of each segment is derived from the same physics as the nuclear bond energy solver (\texttt{bond\_energy\_solver.py}, line~245): the bond-region dielectric $\varepsilon_\text{bond} = n_e / \alpha$, giving:
\begin{equation}
    Z_\text{seg} = \sqrt{\mu / \varepsilon} \;\propto\; \frac{1}{\sqrt{n_e}}
    \label{eq:z_seg_electron}
\end{equation}
\begin{center}
\begin{tabular}{@{}l c c r@{}}
\toprule
\textbf{Bond} & \textbf{Type} & $n_e$ & $Z = 1/\sqrt{n_e}$ \\
\midrule
N--C$_\alpha$ & single & 2 & 0.707 \\
C$_\alpha$--C & single & 2 & 0.707 \\
C--N (peptide) & partial double & 3 & 0.577 \\
\bottomrule
\end{tabular}
\end{center}
The 22\% impedance mismatch at each peptide bond junction is the protein-scale analog of a semiconductor band-gap junction: it creates strong reflections that make $S_{11}$ sensitive to local geometry.
...
\textbf{Current status:} The H-bond term is physically correct and adds non-zero directional coupling, but does not significantly alter the $\alpha/\beta$ secondary structure distribution at Villin HP35 scale (24\% total SS with or without the term).  The dominant backbone geometry penalties ($\lambda_\text{bond} = 2.0$, $\lambda_\text{rama} = 1.79$) control the $\phi/\psi$ landscape; the H-bond shunt admittance contributes at a lower energy scale ($\kappa_\text{HB} = 0.071$).  Future work will investigate whether multi-body H-bond cooperativity (where a chain of consecutive H-bonds progressively stabilises the $\alpha$-basin) can break the $\alpha/\beta$ degeneracy.
