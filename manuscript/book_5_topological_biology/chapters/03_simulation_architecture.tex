...
The engine correctly enforces backbone chirality (100\% negative $\phi$) and produces compact structure ($R_g = 4.5$~\AA), but favours the $\beta$-basin ($\phi \approx -120^\circ$, $\psi \approx 130^\circ$) over the $\alpha$-basin ($\phi \approx -60^\circ$, $\psi \approx -40^\circ$).  The residual $\alpha/\beta$ selectivity requires explicit hydrogen-bonding impedance (N--H$\cdots$O=C cross-coupling)---an identified next upgrade that would bias $i \to i{+}4$ contacts over $i \to i{+}2$.
