...
\midrule
Loss & $340 \to 3.33$ & Convergent \\
$R_g$    & 4.4~\AA & 8--10~\AA \\
End-to-end & 13.8~\AA & $<30$~\AA~\checkmark \\
$\phi$ chirality & 100\% negative & \checkmark \\
$\alpha$-helix & 42\% & $\sim\!60\%$ \\
$\beta$-sheet  & 48\% & --- \\
\textbf{Total SS} & \textbf{91\%} & $>70\%$~\checkmark \\
\bottomrule
\end{tabular}
\end{center}
...
\end{equation}
This forces each residue to commit to a single consistent $(\phi,\psi)$ basin.  The improvement was dramatic: 24\% $\to$ 91\% total secondary structure (Table above).  The 42\%/48\% $\alpha/\beta$ balance reflects the equal-depth double-well; future sequence-dependent basin asymmetry from the per-residue impedance $|Z_\text{topo}|$ may further improve the $\alpha$-helix fraction.
