...
$R_g$    & 8.5~\AA & 8--10~\AA~\checkmark \\
End-to-end & 23.2~\AA & $<30$~\AA~\checkmark \\
Helix content & 15\% & $\sim\!60\%$ \\
\bottomrule
\end{tabular}
\end{center}

The engine correctly predicts the \emph{size} of the native fold ($R_g = 8.5$~\AA{} vs.\ experimental $\sim\!9.8$~\AA) but underestimates helical content.  The C$_\alpha$-only representation lacks explicit backbone dihedral constraints ($\phi, \psi$); without these, the optimizer can satisfy the $S_{11}$ objective through tight folds rather than regular helices.  Adding Ramachandran basin enforcement---derived from the peptide bond geometry (Axioms 1--2)---is the identified next step for improving secondary structure fidelity.
