\chapter{Simulation Architecture: S\texorpdfstring{$_{11}$}{11} Protein Folding Engine}
...
\begin{equation}
    \boxed{\mathcal{L}(\mathbf{r}) = \underbrace{\overline{|S_{11}|^2}}_{\text{impedance mismatch}} + \underbrace{\lambda_b \sum_i (d_i - d_0)^2 / N}_{\text{bond integrity}} + \underbrace{\lambda_s \sum_{|i-j|\geq 3} \max(0, r_{\text{steric}} - d_{ij})^2 / N}_{\text{Pauli exclusion}} + \underbrace{\mathcal{L}_{\text{port}}}_{\text{cavity coupling}}}
    \label{eq:total_loss}
\end{equation}
...
$D_\text{H_2O}$ & 2.75~\AA  & Water molecular diameter                  & Axiom 2 $\to$ O--H bond length \\
$\beta_\text{burial}$ & 1.6~\AA$^{-1}$ & $4.4 / D_\text{H_2O}$ (sigmoid 10--90\% = $D_\text{H_2O}$) & Derived \\
$r_\text{steric}$ & 3.4~\AA & $2 \times r_{C_\alpha}$ (Pauli exclusion) & Axiom-derived Slater radii \\
$Z_\text{H_2O}$ & $\sqrt{80} \approx 8.9$ & $\sqrt{\varepsilon_r}$ for liquid water & Axiom 1 $\to$ dielectric theory \\
...
\begin{equation}
    Y_i^{\text{solvent}} = \frac{e_i}{Z_\text{H_2O}}, \qquad e_i = \operatorname{clip}\!\left(1 - \frac{n_i}{n_\text{max}},\; 0,\; 1\right)
