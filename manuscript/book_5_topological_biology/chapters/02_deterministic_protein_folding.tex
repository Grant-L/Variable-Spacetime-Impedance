...
\begin{table}[H]
\centering
\caption{RMSD comparison: eight-force engine vs $S_{11}$ minimiser with conjugate impedance matching and solvent boundary.  The $S_{11}$ engine uses zero force constants---every structural feature emerges from impedance matching.}
\label{tab:s11_comparison}
\small
\begin{tabular}{@{}l c r r r r@{}}
\toprule
\textbf{Peptide} & \textbf{PDB} & \textbf{8-Force ({\AA})} & \textbf{$S_{11}$+Solvent ({\AA})} & \textbf{$\Delta$} & $R_g^{S_{11}} / R_g^{\text{PDB}}$ \\
\midrule
Chignolin  & 5AWL & 4.34 & \textbf{3.41} & $-21\%$ & 5.5 / 4.8 \\
Trpzip2    & 1LE1 & 5.58 & \textbf{5.21} & $-7\%$  & 6.4 / 5.8 \\
Trp-cage   & 1L2Y & 6.56 & \textbf{5.48} & $-16\%$ & 6.8 / 7.0 \\
Villin     & 1YRF & \textbf{7.31} & 8.43      & $+15\%$ & 10.4 / 8.9 \\
\midrule
\textbf{Mean} & & 5.95 & \textbf{5.63} & $-5\%$ & \\
\bottomrule
\end{tabular}
\end{table}
...
The final engine combines five physics layers: (1)~Adam adaptive learning rates (optax), which handle the mixed-scale landscape where some residues need large moves and others need precision; (2)~multi-frequency $S_{11}$ integration over 5 frequencies spanning $0.5$--$2.0 \times \omega_0$, which smooths the loss landscape; (3)~simulated annealing with quadratic temperature cooldown over the first 50\% of steps, which escapes local minima; (4)~conjugate impedance matching with $Q$-factor scaling, which captures salt bridges and hydrophobic pairing through a single coupling formula; and (5)~a solvent impedance boundary, where every exposed residue couples to the aqueous environment ($Z_{\text{H}_2\text{O}} \approx \sqrt{\varepsilon_r} \approx 9$) through a smooth sigmoid parasitic shunt admittance. Using \texttt{jax.lax.fori\_loop} for the ABCD cascade eliminates Python loop unrolling, reducing JIT compilation to $< 1$~s for all chain lengths.

For peptides $N \leq 20$, the $S_{11}$ minimiser achieves consistently lower RMSD than the eight-force engine with zero free parameters. Trp-cage achieves $R_g = 6.8 / 7.0$~\AA\ (97\% match to PDB). For Villin HP35 ($N = 35$), the solvent impedance boundary reduces the RMSD from 9.5~\AA\ to 8.4~\AA\ and compresses $R_g$ from 12.2 to 10.4~\AA\ (PDB = 8.9~\AA). The remaining gap reflects the difficulty of tertiary topology: the $S_{11}$ loss converges well, but the 3-helix bundle packing requires longer-range correlations that the current pairwise shunt coupling does not yet capture.
...
\begin{enumerate}
    ...
    \item \textbf{RMSD $\sim 5.6$~\AA.} The model correctly predicts secondary structure type and produces physically realistic $R_g$ values (Trp-cage $R_g$ within 3\% of PDB), but per-residue coordinate accuracy (mean RMSD $\approx 5.6$~\AA) lags behind empirical methods. The remaining error is dominated by tertiary topology: multi-helix bundles require longer-range correlations beyond pairwise coupling.
...
\begin{itemize}
    ...
    \item \textbf{Multi-Scale Coupling.} Tertiary bundle packing (e.g., Villin 3-helix) requires correlations beyond pairwise shunt admittance. A multi-port $S$-parameter model, where impedance discontinuities define port boundaries and inter-port scattering drives segment packing, could resolve this.
