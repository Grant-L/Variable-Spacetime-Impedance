...
Certain sidechains map to a low topological impedance coefficient ($Z_{topo} < 1.0$), allowing the backbone atoms to pack tightly and curl into the balanced cylindrical geometry of an Alpha-Helix. Conversely, bulky or rigid sidechains map to a high topological impedance coefficient ($Z_{topo} > 1.0$). These sequences repel adjacent backbone nodes, forcing the structure to uncoil and flatten into an extended Beta-Sheet to minimise local geometrical strain.
...
\begin{figure}[H]
    ...
    \caption{\textbf{Topological Gradient Descent (Beta-Sheet):} For \textit{Polyvaline} ($Z_{topo} \approx 3.8$), the high topological mismatch penalises curvature, exerting steric pressure. The sequence uncoils, flattening into an extended Beta-Sheet geometry to minimise local spatial strain.}
