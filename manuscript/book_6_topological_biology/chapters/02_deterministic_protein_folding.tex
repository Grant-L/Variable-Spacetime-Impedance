...
Salt bridge (Glu--Lys) & Conjugate impedance match ($Z_i Z_j^* > 0$) & $\operatorname{Re}(Z_i \cdot Z_j^*) > 0$ \\
Solvent (water) & Chassis ground (parasitic shunt) & $Y_{\text{solv}} = \text{exposure}_i / Z_{\text{H}_2\text{O}}$ \\
Hydrophobic burial & Stage shielding from ground & Low exposure $\to$ low $Y_{\text{solv}}$ \\
Helix segment & Amplifier stage (matched TL section) & Uniform $Z_{topo}$ segment \\
Turn residue (Gly, Pro) & Inter-stage matching network & Impedance discontinuity ($Z$ ratio $> 4\times$) \\
Multi-helix bundle & Multistage amplifier cascade & $N$-pole coupled resonator \\
Solvent-accessible surface & Parasitic capacitance to ground & $\sum_i Y_{\text{solv},i}$ \\
L-amino acid chirality & Non-reciprocal waveguide (ferrite phase) & $\beta_{\text{eff}} = \beta_0 - \delta_\chi \tanh(\chi/\chi_0)$ \\
...
\medskip\noindent
\textit{For the full axiom-to-backbone derivation chain
($\ell_{node} \to d_p \to d_0$), amino acid impedance table, and
inter-residue $\Gamma$ analysis, see \S\ref{sec:protein_bridge} in
