...
\subsection{The Absolute $W$ Boson Mass}

The mixing angle ratio establishes the \emph{relative} weak boson masses. The \emph{absolute} $W$ mass is determined by the evanescent cutoff of the torsional sector. The characteristic Cosserat length $l_c$ is suppressed relative to $\ell_{node}$ by two dielectric screening factors: $\alpha^2$ (the squared compliance of the LC matrix) and $p_c = 8\pi\alpha$ (the packing fraction that governs volumetric energy storage). The torsion--shear coupling projects through $\sin\theta_W$. Combining these:
\begin{equation}
    M_W = \frac{m_e}{\alpha^2 \, p_c \, \sin\theta_W} = \frac{m_e}{8\pi\alpha^3 \sin\theta_W} \approx 79{,}923 \text{ MeV}
\end{equation}
This matches the CODATA value ($80{,}379 \pm 12$ MeV) to within $0.57\%$. Combined with the mixing angle:
\begin{equation}
    M_Z = \frac{3}{\sqrt{7}} M_W \approx 90{,}624 \text{ MeV} \qquad (0.62\% \text{ from CODATA } 91{,}188 \text{ MeV})
\end{equation}

No adjustable parameters appear: $\alpha$, $p_c$, and $\sin^2\theta_W = 3/7$ are all independently derived geometric constants of the Chiral LC lattice.
...
\begin{figure}[h]
    \centering
    \includegraphics[width=1.0\textwidth]{../../assets/sim_outputs/electroweak_acoustic_modes.png}
