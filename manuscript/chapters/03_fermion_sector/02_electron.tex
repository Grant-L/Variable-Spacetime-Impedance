...
Since the vacuum is a discrete lattice with pitch $l_{node}$, all geometric integrals must be normalized by the fundamental hardware voxel size to yield dimensionless Impedance Shape Factors ($\hat{\Lambda}$):
\begin{align}
    \hat{\Lambda}_{vol} &= \frac{1}{l_{node}^3} \iiint_{V} dV = 4\pi^3 \quad (\text{Dimensionless Node Count}) \\
    \hat{\Lambda}_{surf} &= \frac{1}{l_{node}^2} \iint_{S} dA = \pi^2 \quad (\text{Dimensionless Surface Flux}) \\
    \hat{\Lambda}_{line} &= \frac{1}{l_{node}} \int_{L} dl = \pi \quad (\text{Dimensionless Path Weight})
\end{align}
