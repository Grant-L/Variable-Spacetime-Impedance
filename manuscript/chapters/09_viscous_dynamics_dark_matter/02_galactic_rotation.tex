...
\label{subsec:flat_rotation}

We model the galaxy using the Navier-Stokes equations for the vacuum substrate in a rotating reference frame. To maintain a flat rotation curve without invoking dark matter, we introduce a \textbf{Viscous Coupling Frequency} ($\omega_{gal}$), which represents the characteristic rotational update rate of the galactic core coupling to the lattice.

The tangential velocity $v(r)$ is derived from the radial momentum balance:

\begin{equation}
    v(r) = \sqrt{\frac{GM}{r} + \nu_{vac} \cdot \omega_{gal} \cdot r_{scale}}
...
\begin{itemize}
    ...
    \item $\nu_{vac} = \frac{\eta}{\rho}$: The Kinematic Viscosity of the vacuum substrate ($m^2/s$).
    \item $\omega_{gal}$: The angular frequency of the galactic coupling ($rad/s$).
    \item $r_{scale}$: A characteristic length scale of the vortex coupling (approximated as unity for $r \gg r_{core}$).
\end{itemize}

Dimensional Analysis check:
\begin{itemize}
    \item Gravitational Term ($\frac{GM}{r}$): $[L^3 T^{-2} M^{-1} \cdot M \cdot L^{-1}] = [L^2 T^{-2}]$ (Velocity squared).
    \item Viscous Term ($\nu \omega$): $[L^2 T^{-1}] \cdot [T^{-1}] = [L^2 T^{-2}]$ (Velocity squared).
\end{itemize}

\textbf{Asymptotic Behavior:}
\begin{enumerate}
    \item \textbf{Inner Region ($r \rightarrow 0$):} Gravity dominates ($\frac{GM}{r} \gg \nu \omega$). The system exhibits standard Keplerian rotation ($v \propto r^{-1/2}$).
    \item \textbf{Outer Region ($r \rightarrow \infty$):} The gravitational term vanishes. The velocity asymptotically approaches a constant floor determined by the substrate viscosity:
    \begin{equation}
        v_{flat} \approx \sqrt{\nu_{vac} \omega_{gal}}
    \end{equation}
\end{enumerate}

Result: The rotation curve flattens naturally. We do not need ``Dark Matter''; we simply need to account for the \textbf{Viscous Floor} imposed by the fluid dynamics of the vacuum.

\begin{figure}[h]
    \centering
    \includegraphics[width=0.9\textwidth]{galaxy_rotation_viscous.png}
    \caption{Galactic Rotation Curve Simulation. The dashed gray line shows the Newtonian prediction (decaying). The solid blue line shows the VSI Navier-Stokes prediction, where the vacuum viscosity creates a velocity floor, matching the flat rotation observed in data (red dots).}
...
def plot_galaxy(r, v_newt, v_total, visc_pot):
    ...
    plt.plot(r, v_total, '-', color='blue', linewidth=3, 
             label='VSI Navier-Stokes Prediction')
