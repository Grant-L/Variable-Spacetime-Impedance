...
While the scalar potential ($\phi_n$) defined in Axiom 2 rigorously governs the longitudinal compression of the lattice (Gravity and Electrostatics), it cannot cleanly represent transverse wave propagation (Photons). To map the transverse $LC$ properties of the $M_A$ manifold to a continuous field theory without dimensional violations, the canonical field variable must be the \textbf{Magnetic Vector Potential} ($\mathbf{A}$), defined physically as the magnetic flux linkage per unit length, measured in Webers per meter ($[\text{Wb/m}] = [\text{V} \cdot \text{s} / \text{m}]$).
