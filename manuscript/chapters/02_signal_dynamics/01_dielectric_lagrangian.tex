...
\subsection{The Variable Dictionary: Unifying the Field Formalisms}
To model the full dynamics of the $M_A$ lattice, we must distinguish between the state of the nodes and the transmission along the edges. This resolves the apparent duality between scalar fields and vector gauge theories:
\begin{itemize}
    \item \textbf{The Scalar Node Potential ($\phi$):} Represents the longitudinal energetic state (Voltage) localized at a specific \textit{Node} (analogous to the $A_0$ gauge component). It governs electrostatic pressure, dielectric saturation, and gravity.
    \item \textbf{The Vector Link Variable ($U_{ij}$):} Represents the transverse phase transport (Current/Flux) across the \textit{Edges} connecting the nodes (analogous to the vector $\vec{A}$ component). It governs magnetic helicity and gauge symmetries (detailed in Chapter 6).
\end{itemize}
