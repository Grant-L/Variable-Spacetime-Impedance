...
\begin{enumerate}
    ...
    \item \textbf{Metric Flux ($\Phi_{ij}$):} A vector link variable representing the momentum density state of the edge ($\Phi \equiv \mathbf{p}$).
\end{enumerate}
...
\mathcal{L}_{node} = \underbrace{\frac{1}{2} C_{eff}(\Delta \phi) \sum_{j \in \text{neigh}(n)} (\phi_n - \phi_j)^2}_{\text{Potential ($CV^2$)}} - \underbrace{\frac{1}{2} (L_{node} C_{node}^2) (\partial_t \phi_n)^2}_{\text{Kinetic ($L I^2$)}}
\end{equation}
...
The vacuum is a non-linear dielectric. The effective capacitance $C_{eff}$ is not constant but is a function of the local potential gradient relative to the **Lattice Tension Limit** ($T_{max}$):
\begin{equation}
\label{eq:saturation_op}
C_{eff}(\Delta \phi) = \frac{C_0}{\sqrt{1 + \left(\frac{\Delta \phi}{T_{max}}\right)^4}}
\end{equation}
where $T_{max} \equiv l_{node} / C_g$ is the vacuum breakdown limit. Note that under the Ansatz, Potential (Volts) and Tension (Newtons) are topologically equivalent ($1\text{ V} \equiv 1\text{ N}$).
\end{axiom}
