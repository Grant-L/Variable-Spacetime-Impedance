...
\textit{Calibration Note:} Matching the derived Planck scale sets $l_0 \sim 10^{-35}\,\mathrm{m}$ (order-of-magnitude). In AVE $l_{0}$ is a primary input parameter of the mesh, not a secondary derivative of constants $G$ and $\hbar$.
...
While the photon performs a random walk at the micro-scale (The Jagged Path), the Graph Laplacian ($\mathcal{L}$) converges to the continuous Laplace-Beltrami operator ($\nabla^2$) at the macro-scale. The vacuum looks smooth to us for the same reason a sandy beach looks smooth from an airplane: the grains (lattice pitch $l_0$) are stochastic, and the signal is gyroscopically stabilized.

\textbf{Physical Result:} Light travels at the same speed in every direction. The vacuum looks smooth to us for the same reason a sandy beach looks smooth from an airplane: the grains (lattice pitch $l_0$) are stochastic and infinitesimally small.
