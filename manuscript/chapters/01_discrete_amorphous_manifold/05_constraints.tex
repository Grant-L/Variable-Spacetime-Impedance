...
\subsubsection{Derived Gravitational Coupling (The Stiffness Ansatz)}
To connect the microscopic lattice yield strength to macroscopic gravity, we introduce a \textbf{Compliance Mapping}. We postulate that the Einstein gravitational coupling constant ($c^4/G$) represents the effective macroscopic stiffness of the vacuum substrate.

We define the substrate stiffness-to-curvature conversion by equating the yield force ($F_{yield} = E_{sat}/l_0$) to the GR stiffness scale:
\begin{equation}
    \frac{c^4}{G_{AVE}} \equiv \kappa_G F_{yield} = \kappa_G \frac{E_{sat}}{l_{0}}
\end{equation}
Here $\kappa_G \sim \mathcal{O}(1)$ is the coarse-graining factor encoding how microscopic yield translates to macroscopic curvature response. This mapping allows us to recover the weak-field Poisson limit while treating $G$ as a derived bulk modulus rather than a fundamental scalar.