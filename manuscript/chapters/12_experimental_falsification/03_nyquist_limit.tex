...
\begin{equation}
\partial_t^2 \phi_n = \frac{c^2}{l_0^2} \sum_{j} (\phi_j - \phi_n)
\end{equation}
For a plane wave solution $\phi(x,t) = A e^{i(kx - \omega t)}$ traversing a lattice with mean pitch $l_0$, the discrete Laplacian operator induces a frequency-dependent dispersion relation. In the simplest 1D approximation (Von Neumann Stability Analysis):
\begin{equation}
\omega(k) = \frac{2c}{l_0} \sin\left(\frac{k l_0}{2}\right)
\end{equation}
...
\begin{equation}
v_g(k) = c \cos\left(\frac{k l_0}{2}\right)
\end{equation}
We now apply the \textit{Continuum Limit} where the wavelength $\lambda$ is macroscopic compared to the lattice pitch ($\lambda \gg l_0$, or $k l_0 \ll 1$). Expanding the cosine term:
\begin{equation}
v_g(k) \approx c \left[ 1 - \frac{1}{8}(k l_0)^2 + \mathcal{O}(k^4) \right]
\end{equation}

\subsubsection{Recovering the Continuum (IR Fixed Point)}
For all standard physical processes (Standard Model physics), the energy scale $E$ is orders of magnitude below the Planck scale breakdown voltage ($l_0 \approx 10^{-35}$ m).
\begin{equation}
k l_0 \approx \frac{10^{-18} \text{ m}}{10^{-35} \text{ m}} = 10^{-17} \approx 0
\end{equation}
Consequently, the dispersion term $\frac{1}{8}(k l_0)^2$ vanishes.
\begin{equation}
\lim_{k \to 0} v_g(k) = c
\end{equation}
...
\begin{equation}
\Delta t_{arrival} \approx \frac{L}{c} \cdot \frac{1}{8} (k l_0)^2
\end{equation}
\textbf{Kill Switch:} If the vacuum is a discrete lattice, high-energy Gamma Ray Bursts (GRBs) should arrive slightly \textit{later} than their low-energy counterparts emitted simultaneously, due to the $\cos(kl_0)$ slowing factor.
\begin{itemize}
...
\end{itemize}
Current observations (Fermi LAT) constrain $l_0 < 1.6 \times 10^{-35}$ m. If future detectors measure a strictly energy-independent speed of light even at the Planck scale, the Discrete Manifold hypothesis is falsified.