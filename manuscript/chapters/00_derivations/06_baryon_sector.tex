...
\textbf{The Self-Consistent Mass Oscillator:} To mathematically convert this pure topological volume into physical mass, it must be scaled by the QED Volumetric Packing Fraction ($\kappa_V = 8\pi\alpha \approx 0.1834$) derived in Chapter 2, and the inductive mass-stiffening ratio ($x = m_p/m_e$). Because the structural tension generating the tensor mass is strictly driven by the total mass of the knot, the mass generation forms a non-linear, self-consistent structural feedback loop.

We can completely remove empirical mass parameters from the derivation by establishing the target mass ratio ($x$) strictly as a Geometric Structural Eigenvalue that must perfectly satisfy the total mass equation:
\begin{equation}
x = \mathcal{I}_{scalar} + \mathcal{I}_{tensor}(x) = \mathcal{I}_{scalar} + \left[ (\text{Total Geometric Volume}) \cdot \kappa_V \cdot x \right]
\end{equation}

Substituting the derived geometric constants yields a strict linear algebraic equation:
\begin{equation}
x = 1162 + (1.974 \cdot 0.1834) \cdot x \implies x = 1162 + 0.36203x
\end{equation}
...
\begin{equation}
x(1 - 0.36203) = 1162 \implies x = \frac{1162}{0.63797} \approx 1821.4\ m_e
\end{equation}

By substituting this geometric eigenvalue back into the tensor trace component ($\mathcal{I}_{tensor} = 1821.4\ m_e - 1162\ m_e$), the 3D orthogonal topological interference evaluates strictly to $659.4\ m_e$. The extreme mass of the baryon is not derived from arbitrary phenomenological potentials or circular parameter inputs; it is computationally proven to be the strict, self-consistent geometric eigenvalue of weaving three orthogonal topological flux tubes through an unyielding, highly porous discrete spatial lattice.
...
\begin{equation}
    T_{nuc} = T_{EM}\left(\frac{m_p}{m_e}\right) \approx 0.212\text{ N} \times 1821.4 \approx 386.1\text{ N}
\end{equation}
...
% Replace Eq 6.11, Eq 6.12, and the text between them in Section 6.5.2
\begin{equation}
    \Delta x = \frac{E_{bond}}{T_{nuc}} = \frac{7.55 \times 10^{-13}\text{ J}}{386.1\text{ N}} \approx 1.955 \times 10^{-15}\text{ m} = 1.955\text{ fm}
\end{equation}
...
Evaluating this geometric displacement as a continuous mechanical strain over the fundamental 386 fm flux tube yields:
    
\begin{equation}
    \epsilon_{strain} = \frac{\Delta x}{l_{node}} = \frac{1.955\text{ fm}}{386.16\text{ fm}} \approx 0.00506 \implies \textbf{0.51\% Strain}
\end{equation}
