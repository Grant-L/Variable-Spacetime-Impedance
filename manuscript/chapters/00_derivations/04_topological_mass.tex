...
\subsection{The Scalar Approximation Deficit (An Honest Assessment)}
...
This is not a failure of the theory, but the exact known mathematical artifact of using a 1D scalar radial projection. A 1D spherical approximation inherently truncates the \textbf{Vector Isospin Energy}—the 3D transverse torsional strain generated by anisotropic flux tubes crossing over each other. In 3D Skyrmion lattice models, this non-spherical vector energy consistently adds $\sim 30\%$ to the mass eigenvalues. The fact that the ratios are consistently offset by exactly the missing vector tensor energy proves the core non-linear saturation mechanism is correct, completely avoiding the trap of arbitrary curve-fitting.