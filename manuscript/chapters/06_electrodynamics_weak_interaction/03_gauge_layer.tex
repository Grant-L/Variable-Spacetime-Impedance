...
By classical continuum mechanics, pure torsional acoustic stiffness ($k_{torsion}$) is governed by the Shear Modulus ($G_{vac}$) and the polar moment of inertia ($J$). Transverse bending stiffness ($k_{bending}$) is governed exclusively by Young's Modulus ($E_{vac}$) and the area moment of inertia ($I$). For a uniform cylindrical solid bond, geometry dictates $J = 2I$. 

Because the effective mass-energy of an acoustic cutoff mode is directly proportional to the square root of its structural propagation stiffness ($m \propto \sqrt{k}$), the exact geometric ratio of their rest masses is:
\begin{equation}
...
The physical continuous connection between node $i$ and node $j$ is a spatial Flux Tube mathematically described by a unitary link variable $U_{ij}$ that parallel-transports the internal geometric phase state between the vertices. To minimize total stored energy, flux must flow smoothly ($U_{ij} \approx 1$). The simplest gauge-invariant geometric quantity on a graph is the Plaquette (a closed continuous loop) product. Because the $\mathcal{M}_A$ framework is built upon an amorphous Delaunay triangulation, the minimal structural Plaquette is a 3-node triangular cycle: $U_P = U_{ij}U_{jk}U_{ki}$.
