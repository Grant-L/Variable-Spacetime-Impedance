...
A matter particle is defined as a topological defect that imposes a stress potential $\Phi_s = -GM/r$ on the surrounding lattice.
To maintain the Matched Impedance condition ($Z = \sqrt{L/C} = \text{const}$), the lattice must distribute this stress \textit{equipartitionally} across both reactive moduli. This ensures no reflection occurs at the stress boundary.
\begin{enumerate}
    \item \textbf{Inductive Strain (Spatial Curvature):} The lattice stretches, increasing inductance. $\Delta L/L_0 = |\Phi_s|/c^2$.
    \item \textbf{Capacitive Strain (Temporal Dilation):} The dielectric stiffens, increasing capacitance. $\Delta C/C_0 = |\Phi_s|/c^2$.
\end{enumerate}
The Total Metric Deformation $\chi$ experienced by a traversing signal is the sum of both constitutive strains:
\begin{equation}
    \chi(r) = 1 + \frac{\Delta L}{L} + \frac{\Delta C}{C} = 1 + \frac{GM}{rc^2} + \frac{GM}{rc^2} = 1 + \frac{2GM}{rc^2}
\end{equation}
\textbf{Result:} The Schwarzschild factor of 2 emerges strictly from the requirement of impedance matching (energy equipartition) in the medium, without invoking tensor calculus.
