...
\begin{itemize}
    ...
    \subsubsection{Axiom V: The Lattice Relaxation Threshold}
    ...
    Where $a_0 \approx 1.2 \times 10^{-10} m/s^2$. Below this acceleration (Deep Space), the lattice "relaxes," increasing the effective inductive coupling. \citestart This non-linearity is the mechanical origin of the "Dark Matter" signal observed in galactic rotation curves\cite{1035}\citeend.

\end{itemize}

\subsection{Electrodynamics: The Lagrangian of the Lattice}
Defining the scalar potential $\phi(x,t)$ (Units: Volts), we construct the Lagrangian Density $\mathcal{L}$ ($J/m^3$) from the capacitive (potential) and inductive (kinetic) energy densities of the transmission line.

To satisfy dimensional homogeneity, the Lagrangian is defined as:
\begin{equation}
    \mathcal{L} = \frac{1}{2}\epsilon(U)(\nabla \phi)^2 - \frac{1}{2}\mu(r)\epsilon(U)^2 \left(\frac{\partial \phi}{\partial t}\right)^2
\end{equation}

Applying the Euler-Lagrange equation yields the constitutive Wave Equation:
\begin{equation}
    \nabla \cdot (\epsilon \nabla \phi) - \mu \epsilon^2 \frac{\partial^2 \phi}{\partial t^2} = 0
\end{equation}
Since $c = 1/\sqrt{\mu\epsilon}$, the time term simplifies to $\frac{1}{c^2}\frac{\partial^2 \phi}{\partial t^2}$, recovering the standard wave equation.
...
\subsection{The Metric Strain Mechanism ($\chi$)}
In VSI, a mass $M$ represents a "Geometric Void" or lattice compression. We model the strain field $\chi(r)$ via Flux Conservation.

\subsubsection{Flux Conservation}
By Gauss's Law for the lattice, the total strain flux $\Psi$ through any shell at distance $r$ is constant:
\begin{equation}
    \Psi = \oint \nabla \chi \cdot dA = 4\pi r^2 \frac{d\chi}{dr} = C
\end{equation}
Solving for the gradient:
\begin{equation}
    \frac{d\chi}{dr} = \frac{C}{4\pi r^2}
\end{equation}

\subsubsection{The Equipartition Derivation}
To determine the constant $C$, we apply the \textbf{Lattice Equipartition Principle}. The energy of the defect must be stored equally in the Inductive ($\mu$) and Capacitive ($\epsilon$) moduli to maintain impedance matching ($Z_0$).

1.  \textbf{Static Potential:} The classic Newtonian potential is $\Phi = -GM/r$.
2.  \textbf{Total Lattice Strain:} Since the lattice must deform both $\mu$ and $\epsilon$ to store this energy without reflection:
    $$ E_{total} = E_{ind} + E_{cap} = 2 \times E_{potential} $$

This factor of 2 implies the effective refractive index gradient is double that of the static potential alone:
\begin{equation}
    \chi(r) = 1 + \frac{2|\Phi|}{c^2} = 1 + \frac{2GM}{rc^2}
\end{equation}

\textbf{Result:} The Schwarzschild metric coefficient ($2GM/c^2$) is derived strictly from the Equipartition of strain energy, independent of General Relativity's geometric assumptions.
