...
\begin{itemize}
    \item \textbf{Tungsten Signal $(\rho \approx 19.3)$:} The model predicts a quadratic rise in phase shift, reaching $\Delta \phi \approx 0.72$ milli-radians at 100,000 RPM\citestart\cite{1298}\citeend. This is well above the noise floor of modern interferometry $(\sim 10^{-6}$ rad).
    \item \textbf{Aluminum Control $(\rho \approx 2.7)$:} The signal drops to $\approx 0.10$ milli-radians\citestart\cite{1300}\citeend. This 7.1x reduction provides a robust "fingerprint" to distinguish the VSI effect from aerodynamic turbulence, which depends on geometry rather than mass.
\end{itemize}
