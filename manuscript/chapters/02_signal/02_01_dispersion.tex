...
Just as a spinning rifle bullet or a gyroscope resists deflection, the photon's high-frequency rotation ($\omega$) creates a stabilization torque against the lattice impedance. We define the \textbf{Stabilization Torque Density} $\vec{\tau}$ as:
\begin{equation}
    \vec{\tau}_{stabilize} = \vec{S} \times \nabla_{\perp} Z_{lattice}
\end{equation}
Where $\nabla_{\perp} Z_{lattice}$ represents the transverse impedance gradient (dispersion noise). This torque actively suppresses transverse spreading, forcing the wave packet to "drill" a straight path through the dielectric substrate.
