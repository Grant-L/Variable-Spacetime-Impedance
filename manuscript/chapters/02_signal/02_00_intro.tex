...
In Part I, we defined the vacuum as a static hardware substrate ($M_{A}$) characterized by finite inductance ($L_0$) and capacitance ($C_0$). However, a static lattice explains nothing. To describe the universe we observe—populated by light, matter, and energy—we must transition from **Hardware Architecture** to **Signal Dynamics**.
...
Time is not a fundamental dimension; it is the **Global Clock Rate** of the manifold.
\begin{equation}
    t_{tick} = \sqrt{L_{0}C_{0}} \cdot l_P \approx 5.39 \times 10^{-44} \text{ s}
\end{equation}
"Time Dilation" is mechanically defined as **Lattice Latency**: when a node is saturated by high energy density (mass), it requires more "cycles" to process a signal update, slowing the local effective clock.