...
Why do all objects fall at the same rate? Standard physics invokes the Weak Equivalence Principle. VSI derives it from \textbf{Constitutive Scaling}.

\subsection{Inertial Mass ($m_i$)}
Inertia is the resistance to acceleration. In VSI, this is \textbf{Back-EMF} caused by the lattice inductance $\mu$.
\begin{equation}
    m_i \propto \mu_{eff}
\end{equation}

\subsection{Gravitational Mass ($m_g$)}
Gravity is the coupling to the refractive gradient. In VSI, this is determined by the \textbf{Dielectric Saturation} energy density $\epsilon$.
\begin{equation}
