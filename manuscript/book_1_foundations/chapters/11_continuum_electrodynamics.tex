...
Before discussing the bulk properties of the universe, we define the transport mechanism. In the continuous limit ($L \gg \ell_{node}$), signal propagation is governed by the classical Maxwell-Heaviside acoustic wave equation:
...
The substitution is direct: wherever the standard wave equation contains the constant $\varepsilon_0$, we replace it with the voltage-dependent $\varepsilon_{eff}(V)$. Because $c_{eff}(V) = 1/\sqrt{\mu_0 \varepsilon_{eff}(V)}$, the local wave speed itself becomes a function of the field amplitude:
...
We define the baseline macroscopic bulk mass density ($\rho_{bulk}$) of the spatial vacuum network. The effective inductive mass of one discrete node is derived from the inductance-mass isomorphism. The permeability $\mu_0$ has dimensions of $[\text{H/m}]$. Under the topo-kinematic isomorphism ($[\Omega] = \xi_{topo}^{-2}[\text{kg/s}]$), inductance maps to mass via $L = \xi_{topo}^{-2} m$. Because the vacuum inductance per unit length is $\mu_0$, the mass of one node spanning $\ell_{node}$ is:
...
We show that Dark Matter is consistent with the network dynamics of a
...
\end{equation}
\end{resultbox}
\begin{resultbox}{Superluminal Lattice Compression Velocity}
\begin{equation}
