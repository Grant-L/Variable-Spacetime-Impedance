...
Table~\ref{tab:trans_protein_solver} provides the complete mapping between biological, electrical engineering, and AVE axiomatic terminology.  Any reader familiar with \emph{one} column can reconstruct the solver in their native domain.
...
\textbf{Op2$_\times$} & $\boldsymbol{\delta E = c_{\min} \times 2\pi\alpha \times E}$
  & \textbf{Ax.~3}
  & \textbf{Topological crossing penalty (\S\ref{sec:op2_crossing})} \\
\bottomrule
\end{tabular}
\end{table}
...
\subsection{Op2 Topological Crossing Penalty}
\label{sec:op2_crossing_ch4}

The dual-formalism analysis (\S\ref{sec:dual_formalism} of
Chapter~\ref{ch:protein_folding}) predicts a fourth loss term
from the Op2 crossing operator:
\begin{equation}
    f(\theta) = \underbrace{\sum_i |\Gamma_i|^2}_{\text{Op 5--6}}
    + \underbrace{|\Gamma_\text{pack}|^2}_{\text{Op 8}}
    + \underbrace{\langle\Gamma_\text{steric}^2\rangle}_{\text{Op 9}}
    + \underbrace{c_{\min} \times \frac{P_C}{4}}_{\text{Op2 crossing}}
    \label{eq:four_scale_loss}
\end{equation}
where $c_{\min}$ is the minimum crossing number of the
backbone knot type (Eq.~\ref{eq:op2_protein}).

\paragraph{Implementation roadmap.}
\begin{enumerate}
    \item \textbf{Knot detection.}  After fold convergence,
          close the backbone trace (N-terminus to C-terminus)
          and compute $c_{\min}$ via the Alexander polynomial.
          Libraries: \texttt{topoly} or \texttt{KnotProt}.
    \item \textbf{Crossing penalty.}  Add $c_{\min} \times P_C/4$
          to the total loss (Eq.~\ref{eq:four_scale_loss}).
          This is a post-convergence correction, not a
          gradient-driving term (crossing number is not
          differentiable).
    \item \textbf{Validation.}  Compare predicted vs.\
          observed crossing numbers across the PDB.
          Target: $c_{\min} = 0$ for $\sim 99\%$ of structures.
\end{enumerate}
