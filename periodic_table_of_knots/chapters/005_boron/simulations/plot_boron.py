...
from ave.matter.atoms import BoronAtom
...
def render_boron():
    ...
    for e in [atom.e1, atom.e2, atom.e3, atom.e4, atom.e5]:
    ...
    outer_x = [e.pos[0]/a0 for e in [atom.e3, atom.e4, atom.e5]]
    outer_y = [e.pos[1]/a0 for e in [atom.e3, atom.e4, atom.e5]]
    ax.scatter(outer_x, outer_y, color='magenta', s=60, zorder=5)

    ax.set_aspect('equal')
    ax.set_title(r"Topological Boron ($^{11}$B) - Trigonal Planar Harmonic Packing", fontsize=15, pad=15)
    fig.colorbar(c, ax=ax, fraction=0.046, pad=0.04).set_label(r'Dielectric Strain', rotation=270, labelpad=20)
    
    out_dir = root_dir / "periodic_table_of_knots" / "chapters" / "005_boron" / "simulations" / "outputs"
    out_dir.mkdir(parents=True, exist_ok=True)
    plt.savefig(out_dir / "boron_strain.png", bbox_inches='tight')

if __name__ == "__main__": render_boron()