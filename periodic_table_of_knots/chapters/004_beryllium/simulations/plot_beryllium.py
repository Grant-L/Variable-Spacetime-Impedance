...
from ave.matter.atoms import BerylliumAtom
from ave.core import constants as k

def render_beryllium():
    atom = BerylliumAtom()
    a0 = atom.a_0
    bounds = a0 * 3.5
    N = 800
    ...
    for e in [atom.e1, atom.e2, atom.e3, atom.e4]:
    ...
    ax.scatter([atom.e3.pos[0]/a0, atom.e4.pos[0]/a0], [atom.e3.pos[1]/a0, atom.e4.pos[1]/a0], color='magenta', s=60, zorder=5)

    ax.set_aspect('equal')
    ax.set_title(r"Topological Beryllium ($^9$Be) - Perpendicular Harmonic Phase-Lock", fontsize=15, pad=15)
    fig.colorbar(c, ax=ax, fraction=0.046, pad=0.04).set_label(r'Dielectric Strain', rotation=270, labelpad=20)
    
    out_dir = root_dir / "periodic_table_of_knots" / "chapters" / "004_beryllium" / "simulations" / "outputs"
    out_dir.mkdir(parents=True, exist_ok=True)
    plt.savefig(out_dir / "beryllium_strain.png", bbox_inches='tight')

if __name__ == "__main__": render_beryllium()