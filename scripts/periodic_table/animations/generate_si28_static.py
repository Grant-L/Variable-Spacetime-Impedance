...
Z, A = 14, 28
bound, grid_res = 140.0, 100
name = "silicon_28"
title = "Silicon-28 ($^{28}Si$): $7\\alpha$ Pentagonal Bipyramid (Equator)"
...
# Render Equatorial Z=0 Plane showing the 5-Alpha Ring
...
# Emphasize in-plane equatorial alphas
for cx, cy, cz in nodes:
    if abs(cz) < 1.0: # Equator node
        depth_scale = 1.0
        alpha_val = 0.9
        ax.scatter(cx, cy, color='#00ffcc', s=300 * depth_scale, marker='+', linewidth=2, alpha=alpha_val)
        ax.scatter(cx, cy, color='#00ffcc', s=100 * depth_scale, edgecolor='#00ffcc', facecolor='none', linewidth=1.5, alpha=alpha_val)
    else: # Polar nodes (out of focus)
        depth_scale = np.exp(-np.abs(cz / (bound/3.0)))
        ax.scatter(cx, cy, color='#0066aa', s=300 * depth_scale, marker='+', linewidth=1, alpha=0.3)
            
ax.set_title(title, color='white', fontsize=16, pad=20)
ax.tick_params(colors='white')
ax.set_xlabel("X-Axis (d)", color='white')
ax.set_ylabel("Y-Axis (d)", color='white')
