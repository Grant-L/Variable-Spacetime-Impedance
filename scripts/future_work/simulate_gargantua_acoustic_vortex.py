    ...
    for step in range(MAX_STEPS):
        ...
        if np.any(crosses):
            ...
            if np.any(hit_disk):
                ...
                doppler = np.clip(doppler, 0.1, 4.0)
                
                # Procedural Texture (Rings + Distance falloff)
                rings = 0.5 + 0.5 * np.sin(final_rc_mag * 12.0)
                base_lum = (1.5 / (final_rc_mag - R_IN + 0.7)) * rings
                base_lum *= doppler**2.5 # Aggressive cinematic beaming
                
                red = np.clip(base_lum * 1.5, 0.0, 1.0)
                green = np.clip(base_lum * 0.9 * (doppler/1.5)**0.5, 0.0, 1.0)
                blue = np.clip(base_lum * 0.4 * (doppler/1.5)**1.5, 0.0, 1.0)
