...
from s_param_network_engine import fold_network
...
for idx, (name, pdb_id, chain, nmax, fold) in enumerate(PROTEINS):
    ...
    try:
        # Use 2D Y-matrix network engine with derived scaling
        # n_steps = D×Q×N×k_adam, n_starts = ⌈D×N/(2πQ)⌉

        ca, z_topo, trace, bb = fold_network(
            seq, n_steps=None, lr=2e-3, n_starts=None)
        dt = time.time() - t0
