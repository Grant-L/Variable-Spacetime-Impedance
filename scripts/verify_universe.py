...
Scans the Abstract Syntax Tree (AST) of the entire codebase to mathematically
prove that no Standard Model empirical parameters are being "smuggled" downstream
to curve-fit the derivations.
"""
import ast
import sys
from pathlib import Path

# The absolute root of the project
PROJECT_ROOT = Path(__file__).parent.parent
SRC_DIR = PROJECT_ROOT / "src" / "ave"

# Banned Libraries (Must derive from geometry, not Standard Model lookups)
BANNED_IMPORTS = ['scipy.constants']

# Banned "Magic Numbers" (These MUST emerge organically from the DAG, never hardcoded)
# Note: We exempt the `if __name__ == "__main__":` blocks so you can print targets.
MAGIC_NUMBERS = {
    137.036: "Inverse Fine Structure Constant (Must be derived from Golden Torus!)",
    376.73: "Impedance of Free Space (Must be derived from Topology!)",
    ...
    14.11: "Age of the universe (Must be derived from Lattice Expansion!)",
    0.1834: "QED Packing Fraction (Must be derived!)",
    299792458.0: "Speed of light (c) (Must be imported from k.C)",
    1.05457e-34: "Planck constant (hbar) (Must be imported from k.H_BAR)",
    6.674e-11: "Gravitational constant (G) (Must be imported from k.G)"
}
...
class AVESyntaxValidator(ast.NodeVisitor):
    ...
    def visit_ImportFrom(self, node):
        if node.module and "scipy.constants" in node.module:
            self.violations.append(f"Line {node.lineno}: ILLEGAL IMPORT: 'from {node.module}'. Standard Model smuggling.")
        self.generic_visit(node)

    def visit_If(self, node):
        # Ignore blocks under `if __name__ == "__main__":` because they contain empirical comparisons
        is_main = False
        try:
    ...
    def visit_Constant(self, node):
        if isinstance(node.value, (int, float)) and not self.in_main_block:
            # Check if a downstream math operation is trying to use a Magic Number
...
def run_verification():
    ...
    for py_file in SRC_DIR.rglob("*.py"):
        ...
        if py_file.name in EXEMPT_FILES:
            validator.in_main_block = True # Masks the float checker for the Root file
    ...
    if global_violations == 0:
        print(f"VERDICT: MATHEMATICALLY PURE.")
