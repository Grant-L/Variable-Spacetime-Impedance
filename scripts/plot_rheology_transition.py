...
def plot_rheological_phase_transition():
    ...
    ax.plot(shear_stresses, np.full_like(shear_stresses, baseline_viscosity),
            ls='--', lw=2, color='#555555', label=r'Continuum Approximation (Newtonian)')
            
    # Plot the AVE Topological Rheology
    ax.plot(shear_stresses, effective_viscosities, color='#ff3366', lw=3,
            label=r'Topological Viscosity ($\eta_{eff}$)')
            
    # Highlight regions
    ax.axvspan(0, tau_yield, color='#1a1a3a', alpha=0.5, label='Rigid Matrix (Dark Matter)')
    ax.axvspan(tau_yield, tau_yield * 3.0, color='#0a2a1a', alpha=0.5, label='Frictionless Superfluid (Planetary Orbit)')
    
    ax.axvline(tau_yield, color='#00ffcc', ls=':', lw=2, label=r'Avalanche Threshold ($\tau_{yield}$)')
    
    # Formatting
    ax.set_title(r"Bingham-Plastic Dielectric Rupture ($\mathcal{M}_A$ Fluidics)", color='white', weight='bold', fontsize=14)
