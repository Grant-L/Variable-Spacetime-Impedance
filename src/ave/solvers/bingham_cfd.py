...
class BinghamFluidSolver:
    ...
    def update_viscosity_field(self):
        ...
        dynamic_mu = self.nu_solid * self.g.rho + (self.tau_yield / gamma_safe) * (
            1.0 - np.exp(-m_regulizer * gamma_safe)
        )
    ...
    def apply_momentum_diffusion(self):
        ...
        lap_vx = (np.roll(self.g.vx, 1, axis=0) + np.roll(self.g.vx, -1, axis=0) - 2 * self.g.vx) / self.g.dx**2 + (
            np.roll(self.g.vx, 1, axis=1) + np.roll(self.g.vx, -1, axis=1) - 2 * self.g.vx
        ) / self.g.dy**2
        if self.g.Nz > 1:
        ...
        lap_vy = (np.roll(self.g.vy, 1, axis=0) + np.roll(self.g.vy, -1, axis=0) - 2 * self.g.vy) / self.g.dx**2 + (
            np.roll(self.g.vy, 1, axis=1) + np.roll(self.g.vy, -1, axis=1) - 2 * self.g.vy
        ) / self.g.dy**2
        if self.g.Nz > 1:
        ...
        lap_vz = (np.roll(self.g.vz, 1, axis=0) + np.roll(self.g.vz, -1, axis=0) - 2 * self.g.vz) / self.g.dx**2 + (
            np.roll(self.g.vz, 1, axis=1) + np.roll(self.g.vz, -1, axis=1) - 2 * self.g.vz
        ) / self.g.dy**2
        if self.g.Nz > 1:
