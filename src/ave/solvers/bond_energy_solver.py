...
class BondFDTD1D:
    ...
    def place_nuclear_defect(self, site: int, mass_kg: float, n_electrons: int = 0):
        """
        ...
        if n_electrons > 0:
            eps_enhancement = n_electrons * (1.0 / ALPHA)
            for offset, weight in spread.items():
                idx = site + offset
                if 0 <= idx < self.n:
...
def compute_bond_energy_curve(
    ...
    iso_a.place_nuclear_defect(lattice_padding, mass_a_kg, n_electrons=0)
    E_iso_a = iso_a.structural_energy()

    iso_b = BondFDTD1D(2 * lattice_padding)
    iso_b.place_nuclear_defect(lattice_padding, mass_b_kg, n_electrons=0)
    ...
    for i, d in enumerate(d_range):
        ...
        solver.place_nuclear_defect(site_a, mass_a_kg, n_electrons=n_e_each)
        solver.place_nuclear_defect(site_b, mass_b_kg, n_electrons=n_e_each)
