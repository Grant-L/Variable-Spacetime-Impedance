...
class StellarLayer:
    ...
    def impedance_ratio(self) -> float:
        ...
        # Z ∝ 1/√(n_e) in the plasma regime
        n_ref = 1e6  # Reference: interplanetary space
        return 1.0 / np.sqrt(1 + self.n_e / n_ref)
...
def build_radial_profile(layers: list = None,
    ...
    n_ref = 1e6
    Z_ratio = 1.0 / np.sqrt(1 + n_e / n_ref)
...
def tachocline_reflection() -> float:
    ...
    Z_rad = 1.0 / np.sqrt(1 + 1e30 / 1e6)
    # Convection zone: n_e ≈ 10²⁸
    Z_conv = 1.0 / np.sqrt(1 + 1e28 / 1e6)
...
def photosphere_reflection() -> float:
    ...
    """
    # Photosphere: n_e ≈ 10²³
    Z_photo = 1.0 / np.sqrt(1 + 1e23 / 1e6)
...
def solar_opacity_from_impedance(r_frac: float,
    ...
    if omega > omega_p:
        ...
        Z_local = 1.0 / np.sqrt(eps_r)  # normalised
