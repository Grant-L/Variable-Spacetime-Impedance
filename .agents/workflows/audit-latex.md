...
- [ ] **Locate the source**: For each `\includegraphics{filename}` or figure environment, find the generating script in `scripts/` (search by filename or caption keywords)
- [ ] **Run the script**: Execute the generating script with `PYTHONPATH=src python <script>` to regenerate the figure
