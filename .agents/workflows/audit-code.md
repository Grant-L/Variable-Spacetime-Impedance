...
Audit the entire physics engine (`src/ave/`), all simulation scripts (`scripts/`, `future_work/`, `periodic_table/simulations/`), and test suite (`tests/`) for code quality, constants compliance, and architectural integrity.
...
All `.py` files in `scripts/` and its subdirectories (book_1 through book_7, future_work, mechanics, infrastructure, periodic_table, standard_model, spice_manual).
...
For every `.py` file in `src/ave/` and `scripts/`:
...
- [ ] Uses `PYTHONPATH=src` compatible imports (`from ave.core.constants import ...`)
- [ ] Saves outputs to `assets/` or `scripts/*/assets/` — not to repo root
- [ ] No `sys.path` hacks — use proper PYTHONPATH
...
- [ ] Verify `make test` passes: `PYTHONPATH=src python -m pytest tests/ -q`
...
- [ ] Scripts in `scripts/` that no longer import from the current engine API are flagged as potentially stale
...
- [ ] **Prediction count**: The "Master Prediction Table" entry count matches the actual output of `scripts/future_work/master_predictions.py`
