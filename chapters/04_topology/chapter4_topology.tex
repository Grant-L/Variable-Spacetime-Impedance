...
\label{sec:proton_molecule}

While the electron represents the minimum stable topological twist, the \textbf{Proton} emerges as a complex \textbf{Topological Molecule}. It is a stable triplet of phase vortices (identified as Quarks) bound by the shared elastic tension of the vacuum substrate \cite.

\begin{axiombox}[The Strong Force as Lattice Tension]
The "Strong Interaction" is not mediated by virtual particles, but by the physical \textbf{Elastic Tension} of the LC lattice nodes as they attempt to reconcile the conflicting phase windings between three adjacent singularities \cite.
\end{axiombox}

The mass of the proton is dominated by the energy stored in these high-tension \textbf{Flux Tubes} (Gluons) \cite. In the amorphous mesh, the energy required to maintain this triplet configuration relative to a single electron twist yields the emergent mass ratio:
\begin{equation}
    \frac{m_p}{m_e} \approx \frac{\text{Binding Energy}_{\text{Triplet}}}{\text{Rest Energy}_{\text{Soliton}}} \approx 1836
\end{equation}
This confirms that the mass of baryonic matter is almost entirely composed of the vacuum's own elastic potential energy \cite.
