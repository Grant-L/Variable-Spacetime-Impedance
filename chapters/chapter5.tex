...
\section{Problems}
\begin{enumerate}
    \item \textbf{Phase Bridge Tension:} If two entangled particles are separated by 1 km, and the "Bridge" is a physical string in the vacuum, why does the tension not pull them together instantly? (Hint: Consider the transverse vs. longitudinal stiffness of the lattice).
    \item \textbf{Kibble-Zurek Density:} The universe cools through the critical temperature $T_c$ at a rate $\tau_Q$. Using the Kibble-Zurek scaling law, derive the expected density of topological defects (monopoles/protons) per cubic meter.
    \item \textbf{Vacuum Freezing:} Calculate the latent heat released if the vacuum underwent a phase transition from Superfluid A to Superfluid B at redshift $z=10$.
\end{enumerate}