...
\begin{enumerate}
    ...
    \item \textbf{Breakdown Limit:} If the vacuum dielectric breakdown occurs at an field strength of $E_{crit} \approx 10^{18}$ V/m (Schwinger Limit), estimate the maximum energy density $U_{max}$ of the lattice before non-linearity sets in.
    \item \textbf{Common-Mode Drift:} Assume the Hubble Constant $H_0 = 70$ km/s/Mpc represents the drift rate of the lattice DC bias. Calculate the fractional change in the breakdown wavelength $\Delta \lambda / \lambda$ per gigayear.
\end{enumerate}