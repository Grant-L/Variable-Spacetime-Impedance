...
from ave.matter.atoms import FluorineAtom, NeonAtom, NitrogenAtom, OxygenAtom
...
def render_element(atom_cls, elem_name, symbol, mass_num, subtitle, chap_num):
    ...
    fig.colorbar(c, ax=ax, fraction=0.046, pad=0.04).set_label(
        r"Dielectric Strain ($V/V_{snap}$)", rotation=270, labelpad=20
    )

    out_dir = (
        root_dir
        / "periodic_table_of_knots"
        / "chapters"
        / f"{chap_num:03d}_{elem_name.lower()}"
        / "simulations"
        / "outputs"
    )
