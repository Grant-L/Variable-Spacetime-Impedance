...
2. Because 60.3 kV > 43.65 kV Bingham limit, the vacuum melts.
...
OUTPUT_DIR = "future_work/chapters/03_applied_fusion/simulations/outputs"
...
def simulate_fusion_dielectric_limits():
    ...
    E_J = T_keV * 1000 * ave_const.E_CHARGE
    d_turn = (ave_const.E_CHARGE**2) / (4 * np.pi * ave_const.EPSILON_0 * E_J)
    F_avg = E_J / d_turn
    V_topo_kV = (F_avg / xi_topo) / 1000
    
    # Exact value at 15 keV
    E_15 = 15.0 * 1000 * ave_const.E_CHARGE
    d_15 = (ave_const.E_CHARGE**2) / (4 * np.pi * ave_const.EPSILON_0 * E_15)
    F_15 = E_15 / d_15
    V_15 = (F_15 / xi_topo) / 1000
    
    ax1.plot(T_keV, V_topo_kV, color='#FFD54F', lw=4, label='Ion Collision Topological Strain ($V_{topo}$)')
    ax1.axhline(43.65, color='#ff3366', lw=2, linestyle='--', label='Vacuum Bingham Yield (43.65 kV)')
    ax1.axvline(15.0, color='#00ffcc', lw=2, linestyle=':', label='ITER/SPARC Ignition Target (15 keV)')
    
    ax1.fill_between(T_keV, 43.65, V_topo_kV, where=(V_topo_kV > 43.65), color='#ff3366', alpha=0.3, label='Superfluid Melt (Strong Force Fails)')
    ...
    ax2.axhline(43.65, color='#FFD54F', lw=2, linestyle='--', label='Bingham Yield Limit (43.65 kV)')
    ax2.fill_between(area_um2, 43.65, V_NIF_kV, color='#4FC3F7', alpha=0.2, label='Superfluid Rayleigh-Taylor Slip')
    ...
    E_J_new = T_req * 1000 * ave_const.E_CHARGE
    d_turn_new = (ave_const.E_CHARGE**2) / (4 * np.pi * ave_const.EPSILON_0 * E_J_new)
    ...
    ax4.axhline(43.65, color='#ff3366', lw=1.5, linestyle=':', label='Bingham Yield Danger Zone')
