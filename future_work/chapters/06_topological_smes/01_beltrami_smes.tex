...
This exact mathematical constraint can be engineered macroscopically. If a superconducting wire into a complex $\mathbf{(p,q)}$ \textbf{Torus Knot}, the macroscopic current is forced to simultaneously wind poloidally ($p$) around the cross-section and azimuthally ($q$) around the central axis.
...
To quantitatively evaluate this macroscopic topological advantage, an un-approximated Biot-Savart computational solver (\texttt{simulate\_smes\_battery.py}) to integrate the continuous external stray flux leaking beyond the structural boundaries of both designs.
