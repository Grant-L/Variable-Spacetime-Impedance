...
class TestEpsilonEff:
    ...
    def test_array_input(self):
        ...
        V = np.array([0.0, 0.5 * V_SNAP, 0.9 * V_SNAP])
    ...
    def test_symmetry(self):
        ...
        assert epsilon_eff(0.5 * V_SNAP) == pytest.approx(
            epsilon_eff(-0.5 * V_SNAP), rel=1e-12
        )
...
class TestLocalWaveSpeed:
    ...
    def test_near_yield_freezes(self):
        ...
        c = local_wave_speed(0.9999 * V_SNAP)
        assert c < 0.15 * C_0
    ...
    def test_monotonically_decreasing(self):
        ...
        V = np.linspace(0, 0.99 * V_SNAP, 50)
