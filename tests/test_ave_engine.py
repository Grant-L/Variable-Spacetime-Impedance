...
# Ensure the AVE package is importable
sys.path.insert(0, os.path.join(os.path.dirname(__file__), '..', 'src'))

from ave.core.constants import (
    C_0, MU_0, EPSILON_0, Z_0, HBAR, M_E, e_charge,
    ALPHA, G, L_NODE, XI_TOPO, P_C, T_EM, V_SNAP,
...
class TestAxiom4Saturation:
    ...
    def test_linear_recovery_epsilon(self):
        ...
        V_small = V_SNAP * 1e-6  # Tiny fraction of yield
    ...
    def test_nonlinear_epsilon_decreases(self):
        """ε_eff must decrease as V increases."""
        V_low = 0.1 * V_SNAP
        V_mid = 0.5 * V_SNAP
        V_high = 0.9 * V_SNAP

        eps_low = epsilon_eff(V_low)
        eps_mid = epsilon_eff(V_mid)
        eps_high = epsilon_eff(V_high)
    ...
    def test_nonlinear_wave_speed_slows(self):
        ...
        c_low = local_wave_speed(0.1 * V_SNAP)
        c_high = local_wave_speed(0.9 * V_SNAP)
    ...
    def test_nonlinear_impedance_increases(self):
        ...
        z_low = impedance_at_strain(0.1 * V_SNAP)
        z_high = impedance_at_strain(0.9 * V_SNAP)
    ...
    def test_saturation_epsilon_approaches_zero(self):
        """At V = 0.9999 V_yield, ε_eff → ≈ 0."""
        V_near = 0.9999 * V_SNAP
        eps = epsilon_eff(V_near)
    ...
    def test_vectorized_epsilon(self):
        ...
        V_array = np.linspace(0, 0.99 * V_SNAP, 100)
