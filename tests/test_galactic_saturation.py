...
class TestAsymptotes:
    ...
    def test_deep_mond_limit(self):
        ...
        expected = np.sqrt(g_test * A0_MOND)
...
class TestGalaxyRotation:
    ...
    def test_flat_velocity_within_10pct(self):
        ...
        r_test = 20 * KPC
        v = ave_rotation_velocity(self.galaxy, r_test, use_saturation=True)
    ...
    def test_curve_is_flat(self):
        ...
        v_inner = ave_rotation_velocity(self.galaxy, 10 * KPC, use_saturation=True)
        v_outer = ave_rotation_velocity(self.galaxy, 25 * KPC, use_saturation=True)
    ...
    def test_saturation_vs_mcgaugh_agreement(self):
        """Both models should agree within 15% at all radii."""
        for r_kpc in [5, 10, 15, 20, 25]:
            r = r_kpc * KPC
            v_sat = ave_rotation_velocity(self.galaxy, r, use_saturation=True)
            v_mcg = ave_rotation_velocity(self.galaxy, r, use_saturation=False)
            ratio = v_sat / v_mcg
            assert 0.85 < ratio < 1.15, \
                f"At r={r_kpc} kpc: v_sat={v_sat/1e3:.1f}, v_mcg={v_mcg/1e3:.1f}"
...
class TestRadialAccelerationRelation:
    ...
    def test_array_input(self):
        ...
        g_obs = radial_acceleration_relation(g_N, use_saturation=True)
